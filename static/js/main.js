...
async function submitAbsensi() {
    ...
    try {
        ...
        if (response.ok && data.success) {
            ...
            showBanner(errorBanner, data.message || 'Terjadi kesalahan saat menyimpan');
            hideBanner(successBanner);
        }
    } catch (err) {
...
async function refreshAuditLog() {
    try {
        ...
        if (!data.log || data.log.length === 0) {
            tbody.innerHTML = '<tr><td colspan="4" class="empty-state">Belum ada data kehadiran</td></tr>';
            return;
        }

        tbody.innerHTML = data.log.map(function (row) {
            let chipClass = '';
            if (row.Status === 'Hadir' || row.Status === 'WFH') chipClass = 'chip--success';
            ...
            return '<tr>' +
                ...
                '<td><span class="chip ' + chipClass + '">' + escapeHtml(row.Status || '') + '</span></td>' +
...
async function initDashboard(period) {
    try {
        const response = await fetch('/dashboard/api/data?period=' + encodeURIComponent(period));
        const data = await response.json();
...
function changeFilter(period, btn) {
    // Update tombol aktif
    const buttons = document.querySelectorAll('#periodFilter .segmented-control__btn');
...
function renderStatusDonutChart(statusData) {
    ...
    const colors = labels.map(function (label) {
        switch (label) {
            case 'Hadir': return '#1E7E34';
            case 'WFH': return '#28A745';
            case 'Izin': return '#B45309';
            case 'Sakit': return '#CC0000';
...
function renderRekapTable(rekapData) {
    ...
    if (!rekapData || rekapData.length === 0) {
        tbody.innerHTML = '<tr><td colspan="5" class="empty-state">Belum ada data untuk ditampilkan</td></tr>';
        return;
    }

    tbody.innerHTML = rekapData.map(function (row) {
        let chipClass = '';
        ...
        return '<tr>' +
            ...
            '<td>' + (row.izin || 0) + '</td>' +
            '<td>' + (row.sakit || 0) + '</td>' +
            '<td><span class="chip ' + chipClass + '">' + escapeHtml(row.kluster || '-') + '</span></td>' +
            '</tr>';
    }).join('');
}
...
function setTone(tone, btn) {
    currentTone = tone;
    const buttons = document.querySelectorAll('#toneControl .segmented-control__btn');
    buttons.forEach(function (b) {
        b.classList.remove('segmented-control__btn--active');
    });
    btn.classList.add('segmented-control__btn--active');
    ...
    try {
        ...
        if (data.match) {
            ...
            if (successBanner) {
                ...
                if (successMsg) successMsg.textContent = data.message || 'Verifikasi berhasil!';
            }
            ...
            if (failBanner) {
                ...
                if (failMsg) failMsg.textContent = data.message || 'Wajah tidak dikenali';
            }
...
function resetFaceModalUI() {
    ...
    if (captureBtn) { captureBtn.style.display = 'flex'; captureBtn.disabled = false; }
    if (retryBtn) retryBtn.style.display = 'none';
